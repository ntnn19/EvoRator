...
def main(features, huang_rates, use_traditional_features, results_dir, estimator, lasso_regularization, rf_n_trees,
    ...
    # calculate average r4s score of neighboring positions
    neigh_df = training_set[
        ['pdb_position', *[f for f in training_set.columns.tolist() if 'neighbor_pos_' in f], 'zr4s_JC']]
    r4_score_d = dict(zip(neigh_df['pdb_position'], neigh_df['zr4s_JC']))
    neigh_d = dict(zip(neigh_df['pdb_position'],
                       neigh_df[[f for f in training_set.columns.tolist() if 'neighbor_pos_' in f]].values.tolist()))
    mean_neigh_r4s_score_d = {}
    for k in neigh_d:
        tmp = []
        for n in neigh_d[k]:
            tmp.append(r4_score_d.get(n, np.nan))
        mean_neigh_r4s_score_d[k] = np.nanmean(tmp)
    training_set['mean_neigh_r4s_score'] = training_set['pdb_position'].map(mean_neigh_r4s_score_d)
    ...
    if estimator == 'SVR':
        ...
        if fit and pred:
            ...
            # pipeline.fit(X_train.iloc[:1000,:], y_train.iloc[:1000])
            ...
            test_set.to_csv(os.path.join(results_dir, f'EvoRator_SVR-FS_C={svr_c},gamma={svr_gamma}_model_PERFGR_test_set_pred_final' + '.csv'))
            F = False
            if F:
                logging.debug(f'saving model')
                joblib.dump(pipeline, os.path.join(results_dir, f'EvoRator_SVR-FS_C={svr_c},gamma={svr_gamma}_model_PERFGR' + '.joblib'))
