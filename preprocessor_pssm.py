...
def predict_new_case(final_df,HUANG_DATA_PATH,HUANG_PSSM_PATH,OUT_PATH,OUT_NAME):
    from tensorflow.python.keras.models import Model, load_model
...
# N_HIDDEN = 147
    ...
    if not os.path.exists(FITTED_PREPROCESSOR_OUTPATH) and not os.path.exists(TRAINED_MODEL_OUTPATH):
        ...
        logging.debug(f'predicting new case\n')
        logging.debug(f'loading preprocessor from {FITTED_PREPROCESSOR_OUTPATH}\n')
        PREPROCESSOR_PSSM = pickle.load(open(FITTED_PREPROCESSOR_OUTPATH, 'rb'))
        logging.debug(f'loading trained model from {TRAINED_MODEL_OUTPATH}\n')
