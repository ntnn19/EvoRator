...
#    flg =True
    ...
    numeric_features_that_appear_object = np.array(numeric_features)[((all_data_clean[numeric_features].dtypes) == np.dtype('O'))]  # remove later, debugging
    print('numeric_features_that_appear_object',all_data_clean[numeric_features_that_appear_object])
    ...
    if model_selection:
        ...
        X_train = all_data_clean[[*numeric_features, *categorical_features]]
        if pssm_prediction:
    ...
    if pssm_prediction:
        if model_selection:
            ...
            model = NNModel_class(input_dim=[X_train_proc.shape[1]], num_classes=y_train.shape[1], n_hidden=n_hidden)  #
            model.fit(X_train_proc, y_train, epochs=50, batch_size=128, verbose=1,validation_data=(X_val_proc, y_val),callbacks=[ES])
            # model.fit(X_train_proc, y_train)
            pred_training_set = model.predict(X_train_proc)
            pred_val_set = model.predict(X_val_proc)
            pred_test_set = model.predict(X_test_proc)
            train_score_to_assert = stats.spearmanr(y_train.values.ravel(), pred_training_set.ravel())
            val_score_to_assert = stats.spearmanr(y_val.values.ravel(), pred_val_set.ravel())
            test_score_to_assert = stats.spearmanr(y_test.values.ravel(), pred_test_set.ravel())
            ...
            logging.debug(f'train {train_score_to_assert}')
            logging.debug(f'val {val_score_to_assert}')
            logging.debug(f'test {test_score_to_assert}')
            clean_test = pd.concat(
                [clean_test, pd.DataFrame(pred_test_set, columns=[AA[0] + '_predicted' for AA in aa])], axis=1)
            clean_train = pd.concat(
                [clean_train, pd.DataFrame(pred_training_set, columns=[AA[0] + '_predicted' for AA in aa])], axis=1)
            clean_val = pd.concat([clean_val, pd.DataFrame(pred_val_set, columns=[AA[0] + '_predicted' for AA in aa])],
                                  axis=1)
            train_score_after_concat = stats.spearmanr(
                clean_train[[*[AA[0] + '_predicted' for AA in aa], *aa]][aa].dropna().values.ravel(),
                clean_train[[*[AA[0] + '_predicted' for AA in aa], *aa]][
                    [AA[0] + '_predicted' for AA in aa]].dropna().values.ravel())
            logging.debug('train_after_concat', train_score_after_concat)
            val_score_after_concat = stats.spearmanr(
                clean_val[[*[AA[0] + '_predicted' for AA in aa], *aa]][aa].dropna().values.ravel(),
                clean_val[[*[AA[0] + '_predicted' for AA in aa], *aa]][
                    [AA[0] + '_predicted' for AA in aa]].dropna().values.ravel())
            logging.debug('val_after_concat', val_score_after_concat)
            test_score_after_concat = stats.spearmanr(
                clean_test[[*[AA[0] + '_predicted' for AA in aa], *aa]][aa].dropna().values.ravel(),
                clean_test[[*[AA[0] + '_predicted' for AA in aa], *aa]][
                    [AA[0] + '_predicted' for AA in aa]].dropna().values.ravel())
            ...
            logging.debug('Training over all data')
            model = NNModel_class(input_dim=[X_train_proc.shape[1]], num_classes=y_train.shape[1], n_hidden=n_hidden)  #
            model.fit(X_train_proc, y_train, epochs=10, batch_size=128, verbose=1)
            logging.debug(f'Training finished')
            logging.debug(f'Saving model and colum transformer')
            model.save_weights(TRAINED_MODEL_OUTPATH)
            model.save(TRAINED_MODEL_OUTPATH.replace('.h5','/'))
            pickle.dump(preprocessor, open(FITTED_PREPROCESSOR_OUTPATH, 'wb'))
