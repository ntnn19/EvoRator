                ...
                if line[0] in tags:
                    if prm:
                        if prm_type in self.parameters:
                            self.parameters[prm_type] = dict(list(self.parameters[prm_type].items()) + list(prm.items()))
                        else:
...
class Glycosylator:
    ...
    def find_glycans(self, patch, resname):
        ...
        glycans = {}
        for graph in list(nx.Graph(G.subgraph(c)) for c in nx.connected_components(G)):
